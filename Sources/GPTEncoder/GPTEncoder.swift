...
public final class GPTEncoder {
    ...
    private let encoder: [String: Int] = {
        return try! JSONSerialization.jsonObject(with: encoder_json.data(using: .utf8)!)
            as! [String: Int]
    }()
    ...
    public func decode(tokens: [Int]) -> String {
        ...
        let arrays = text.map { String($0) }.compactMap { byteDecoder[String($0)] }.map {
            UInt8($0)
        }
    ...
    public func encode(text: String) -> [Int] {
        ...
        for match in matches {
            let token = encodeString(text: match).compactMap { byteEncoder[Int($0)] }.joined(
                separator: "")
